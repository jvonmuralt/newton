...
def eval_particle_contacts(
    ...
    body_w = wp.spatial_bottom(body_v_s)  # angular velocity (now in bottom)
    body_v = wp.spatial_top(body_v_s)     # linear velocity (now in top)
...
def eval_body_joints(
    ...
    if c_parent >= 0:
        ...
        w_p = wp.spatial_bottom(twist_p)                        # angular velocity (now in bottom)
        v_p = wp.spatial_top(twist_p) + wp.cross(w_p, r_p)     # linear velocity (now in top)
