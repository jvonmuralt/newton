...
def apply_mjc_qfrc_kernel(
    ...
    if jtype == JointType.FREE or jtype == JointType.DISTANCE:
        ...
        # extract linear force and angular torque in correct order
        v = wp.vec3(joint_f[wqd_i + 0], joint_f[wqd_i + 1], joint_f[wqd_i + 2])  # linear force
        w = wp.vec3(joint_f[wqd_i + 3], joint_f[wqd_i + 4], joint_f[wqd_i + 5])  # angular torque
...
def eval_single_articulation_fk(
    ...
    for i in range(joint_start, joint_end):
        ...
        if parent >= 0:
            ...
            v_wp = body_qd[parent]
            w_p = wp.spatial_bottom(v_wp)                        # angular velocity (now in bottom)
            v_p = wp.spatial_top(v_wp) + wp.cross(w_p, r_p)     # linear velocity (now in top)
            v_wpj = wp.spatial_vector(v_p, w_p)  # spatial vector with (linear, angular) ordering
        ...
        if type == JointType.PRISMATIC:
            ...
            v_j = wp.spatial_vector(axis * qd, wp.vec3())  # linear velocity first
        ...
        if type == JointType.REVOLUTE:
            ...
            v_j = wp.spatial_vector(wp.vec3(), axis * qd)  # angular velocity second
        ...
        if type == JointType.BALL:
            ...
            v_j = wp.spatial_vector(wp.vec3(), w)  # angular velocity second
        ...
        if type == JointType.D6:
            ...
            v_j = wp.spatial_vector(vel_v, vel_w)  # linear velocity first, angular second
        ...
        angular_vel = wp.transform_vector(X_wpj, wp.spatial_bottom(v_j))  # angular velocity (now in bottom)
        linear_vel = wp.transform_vector(X_wpj, wp.spatial_top(v_j))      # linear velocity (now in top)
