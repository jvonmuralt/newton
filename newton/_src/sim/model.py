...
class Model:
    ...
    def __init__(self, device: Devicelike | None = None):
        ...
        self.meshes = []
        """Mesh pool: list of all registered :class:`Mesh` objects."""
        self.mesh_scales = []
        """Per-pool-mesh scale, one ``np.ndarray(3,)`` per :attr:`meshes` entry."""
        self.shape_mesh_id = None
        """Runtime-writable mesh pool index per shape, shape [shape_count], int32.

        Values are indices into :attr:`meshes`. A value of ``-1`` means the
        shape is not a mesh or should not be swapped.  Write a mesh pool index
        here and call
        :meth:`solver.notify_model_changed(SolverNotifyFlags.SHAPE_MESH_PROPERTIES)
        <newton.solvers.SolverBase.notify_model_changed>`
        to swap which mesh a shape uses at runtime.
        """
