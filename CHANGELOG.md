...
- Add `deterministic` and `deterministic_max_records` constructor arguments to `SolverXPBD`, `SolverSemiImplicit`, `SolverFeatherstone`, `SolverVBD`, and `SolverMuJoCo` to route solver kernel modules through Warp's deterministic codegen without mutating the global config
