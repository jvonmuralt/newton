...
- Add `deterministic` constructor argument to `SolverXPBD`, `SolverSemiImplicit`, `SolverFeatherstone`, and `SolverVBD` (accepts `True`/`False`, `"not_guaranteed"`, `"run_to_run"`, `"gpu_to_gpu"`, or `None` to defer to `wp.config.deterministic`) which routes the solver's atomic-emitting kernel modules through Warp's deterministic codegen without mutating the global config
