...
- Add `deterministic` constructor argument to `SolverXPBD`, `SolverSemiImplicit`, `SolverFeatherstone`, `SolverVBD`, and `SolverMuJoCo` to route solver kernel modules through Warp's deterministic codegen without mutating the global config; `SolverMuJoCo` additionally exposes `deterministic_max_records` (default `16`) because MuJoCo-Warp solver kernels emit deterministic atomics inside data-dependent loops
